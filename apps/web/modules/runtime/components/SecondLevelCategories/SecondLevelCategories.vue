<template>
  <ul class="flex flex-wrap gap-[10px] xl:gap-[20px] justify-center pt-[35px] xl:max-w-[80%] 2xl:max-w-[90%] xl:mx-auto xl:pt-[70px] 2xl:pt-[100px]">
    <li v-for="child in allChildren" :key="child.id">
      <SfTooltip
        :label="'in Kategorie ' + getTooltipText(child) + ' anzeigen'"
        placement="top"
...
import type { SecondLevelCategoriesProps } from './types';
...
const buttonClasses = `
  ...
  text-[14px] xl:text-[18px] 2xl:text-[20px] font-extrabold uppercase no-underline text-black
  transition-colors
