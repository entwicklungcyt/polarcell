<template>
    <div>
      ...
      >
        ...
        <div class="grid grid-cols-[auto_55px] xl:grid-cols-[auto_80px]">
          <div class="prices-badge">
            ...
            <div class="text-[12px] xl:text-[14px] pt-[15px] xl:pt-[25px]">
              ...
              <SfLink :href="localePath('/versandinformationen')" target="_blank" class="!text-primary-500">{{ $t('Produktseite.Versand') }}</SfLink>
            </div>
        ...
        <div class="pt-[30px] xl:pt-[60px]">
          <div class="flex flex-col gap-[10px] xl:gap-[20px]">
            <p class="font-extrabold italic sm:text-[16px] xl:text-[18px] sm:text-center xl:text-left">{{ t('Produktseite.Fragen.01') }}</p>
...
// load data for "Werkzeugset"
const { data: secondProduct, fetchProduct: fetchSecondProduct, loading: loadingSecond } = useProduct('product-5088-6086');

onMounted(async () => {
  await fetchSecondProduct({ id: 5088, variationId: 6086 }); 
});
</script>
