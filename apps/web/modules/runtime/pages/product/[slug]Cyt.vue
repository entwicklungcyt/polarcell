          ...
          <div class="right">
            <div class="drift-zoom-image relative">
              <NuxtLazyHydrate when-idle>
                <UiPurchaseCard v-if="product" :product="product" :review-average="countsProductReviews" :hide-compatibility-checker="true ? breadcrumbs[2]?.name === 'Zubehör' : false" :breadcrumbs="breadcrumbs" />
              </NuxtLazyHydrate>
            </div>
          </div>
        </div>
        ...
        <div>
          <div class="sticky top-[20px] [.slim-header-active_&]:top-[80px]">
            ...
            <div v-if="manufacturerHtml" class="pt-[40px]">
              ...
              <a href="/batterieentsorgung" target="_blank" class="flex items-center gap-[5px] underline pt-[20px] text-[18px] flex items-center gap-[10px] text-black">
...
watch(
  () => product.value,
  () => {
    const image = productGetters.getCoverImage(product.value).replace('/middle/', '/full/');
    ...
    const keywords = product.value?.texts?.keywords || '';
